...
Al principio, los datos que recopilé estaban ordenados en una tabla de Excel (súper básica) con los equipos hacia abajo y los años hacia la derecha. El problema de esto es que, al ingresarlo a un programa como Python, no lee bien estos cruces. Así que lo primero que hice fue limpiar esa base de datos obtenida en la web [EssentiallySports](https://www.essentiallysports.com/f1-news-what-are-the-budgets-for-f1-teams-including-mercedes-red-bull-ferrari/) y transformar esto a una lista larga, donde cada fila es solo un equipo en un año en particular. 
